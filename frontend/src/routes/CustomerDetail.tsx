...
type ApiResp = {
  ...
  summary: Summary
  monthly?: { month: string; qty: number }[] // (fallback if /overview still returns it)
  products: ProductRow[]
}
...
export default function CustomerDetail() {
  ...
  // Load overview (summary, products, customer header)
  useEffect(() => {
    (async () => {
      if (!id) return
      setLoadingMeta(true)
      ...
      try {
        ...
        setData(json)
        // If your existing /overview already returns monthly, use it as a first render
        if (json.monthly && json.monthly.length) {
          setMonthly(json.monthly.map(m => ({ month: m.month.slice(0, 7), total_qty: m.qty })))
        } else {
          setMonthly([]) // will be filled by the dedicated monthly call below
        }
      } catch (e: any) {
  ...
  // Load monthly series with new endpoint & controls
  async function loadMonthly() {
    ...
    try {
      ...
      const res = await fetch(`${(import.meta as any).env.VITE_API_BASE}/api/customers/${id}/monthly?` + params.toString())
  ...
  useEffect(() => { loadMonthly() }, [id, mode, year])
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <Card>
                <CardContent className="p-6">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{data.summary.total_qty.toLocaleString()}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-6">
                  <p className="text-sm text-gray-600">Distinct Products</p>
                  <p className="text-2xl font-bold text-gray-900">{data.summary.distinct_products}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-6">
                  <p className="text-sm text-gray-600">First Purchase</p>
                  <p className="text-2xl font-bold text-gray-900">{data.summary.first_date ?? '—'}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-6">
                  <p className="text-sm text-gray-600">Last Purchase</p>
                  <p className="text-2xl font-bold text-gray-900">{data.summary.last_date ?? '—'}</p>
                </CardContent>
              </Card>
            </div>
            ...
            <Card>
              <CardHeader>
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <Button onClick={loadMonthly} disabled={loadingMonthly}>Refresh</Button>
                </div>
              </CardHeader>
              <CardContent>
